...
in
{
  ...
  systemd.services = lib.concatMapAttrs (_: v: v) (
    lib.genAttrs ethernetDevices (device: {
      "network-addresses-${device}".wantedBy = lib.mkForce [
        "sys-subsystem-net-devices-${device}.device"
      ];
    })
  );
